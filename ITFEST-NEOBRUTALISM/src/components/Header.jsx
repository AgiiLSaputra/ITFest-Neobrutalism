    ...
    <header className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-6xl transition-all duration-300">
      <div className="container mx-auto px-8 h-16 flex items-center justify-between bg-white neo-border neo-shadow-sm">
        ...
        <nav className="hidden md:flex items-center gap-6 font-bold">
          ...
          <button className={navBtn} onClick={() => scrollTo('tentang')}>About Us</button>
          <button className={navBtn} onClick={() => scrollTo('roadmap')}>Proyeksi</button>
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {dropdownOpen && (
              <div className="absolute top-full left-0 mt-1 bg-white neo-border neo-shadow-sm py-2 min-w-[220px] animate-dropdown-pop">
                {eventLinks.map((ev) => (
          ...
          <button className={navBtn} onClick={() => scrollTo('pendaftaran')}>Kemitraan</button>
      ...
      <div className={`md:hidden bg-white neo-border neo-shadow-sm mt-2 transition-all duration-300 ${menuOpen ? 'mobile-menu-open' : 'mobile-menu-closed'}`}>
        <nav className="flex flex-col gap-4 font-bold">
          ...
          <button className="text-sm font-black hover:bg-black hover:text-white px-2 py-1 transition-colors border-2 border-transparent hover:border-black text-left" onClick={() => { scrollTo('tentang'); setMenuOpen(false); }}>About Us</button>
          <button className="text-sm font-black hover:bg-black hover:text-white px-2 py-1 transition-colors border-2 border-transparent hover:border-black text-left" onClick={() => { scrollTo('roadmap'); setMenuOpen(false); }}>Proyeksi</button>
          ...
          <button className="text-sm font-black hover:bg-black hover:text-white px-2 py-1 transition-colors border-2 border-transparent hover:border-black text-left" onClick={() => { scrollTo('pendaftaran'); setMenuOpen(false); }}>Kemitraan</button>
